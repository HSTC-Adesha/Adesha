...
function renderDate() {
    ...
    for (let i = 1; i <= endDate; i++) {
        if (i === today.getDate() && dt.getMonth() === today.getMonth() && dt.getFullYear() === today.getFullYear()) {
            cells += '<div  attr-day="'+moment(i,"D").format("DD")+"/"+moment(dt.getMonth()+1,"M").format("MM")+"/"+dt.getFullYear()+'" class="daytoselect icalendar__today">' + i + '</div>';
        } else {
            cells += '<div  attr-day="'+moment(i,"D").format("DD")+"/"+moment(dt.getMonth()+1,"M").format("MM")+"/"+dt.getFullYear()+'" class="daytoselect">' + i + '</div>';
        }
