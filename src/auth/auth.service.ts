...
export class AuthService {
  ...
  constructor(
    ...
    this.cryptr = new Cryptr('adeshaDB');
  }
...
private async blockUser(user) {
  user.blockExpires = addHours(new Date(), 10);
  await user.save();
}
...
private buildRegistrationInfo(user): any {
 ...
 // this.emailhandlerService.registrationMail(user);
  return userRegistrationInfo;
}
private setRegistrationInfo(user): any {
  user.verification = v4();
  user.verificationExpires = addHours(new Date(), 10);
}
...
private async setUserAsVerified(user) {
  ...
  async createAccessToken(userId: string) {
    ...
    const accessToken = sign({userId},'adeshaDB' , { expiresIn: '30m' });
  ...
  public jwtExtractor(request) {
    ...
    const cryptr = new Cryptr('adeshaDB');
    if (token) {
      try {
